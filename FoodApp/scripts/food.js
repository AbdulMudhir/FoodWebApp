...
function instructionRedirectButton(event)
{
    let link = this.querySelector("a").attributes["href"].value;
...
{
    const instructionButton = document.getElementsByClassName("instructions-button");
    for(i=0; i<instructionButton.length; i++)
    {

        instructionButton[i].addEventListener("click", instructionRedirectButton)
...
function bookmark(event)
{
    ...
    {   
       ...
       removeBookmark(foodTitle);
        ...
        confirmed.textContent = "Removed Bookmark";
        confirmed.style.backgroundColor = "rgb(204,42,54)";
        confirmed.style.opacity = "1";
        confirmed.style.visibility = "visible";
        setTimeout(() => {
            confirmed.style.opacity = "0";
            confirmed.style.visibility = "hidden";
        }, 1000);
    }
    else{
        ...
        confirmed.textContent = "Bookmarked";
        confirmed.style.backgroundColor = "rgb(14, 185, 14)";
        confirmed.style.opacity = "1";
        confirmed.style.visibility = "visible";
...
function parseJson()
{
    ...
    {   
        ...
        if(data.count > 0)
        {
            ...
            food.forEach(content => {

                let recipe = content.recipe;

                let foodTitle = recipe.label;
                ...
                let id =  foodTitle.split(" ").join("")+source.split(" ").join("");
                ...
                recipe.dietLabels.forEach( dietLabel => {

                    healthLabel.push(dietLabel)
                })

                recipe.healthLabels.forEach(health =>{
                    healthLabel.push(health)
                })
                ...
                let nutrition = recipe.totalNutrients;

                let protien = parseInt(nutrition.PROCNT.quantity);

                let carbs = parseInt(nutrition.CHOCDF.quantity);

                let saturates= parseInt(nutrition.FASAT.quantity)

                let fibre = parseInt(nutrition.FIBTG.quantity)
                let salt = parseInt(nutrition.NA.quantity)

                let fat = parseInt(nutrition.FAT.quantity);
                ...
                if (foodTitle in bookmarks.recipes)
                {
